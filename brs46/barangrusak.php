  ...
  <div class="modal-dialog modal-default">
    <div class="modal-content">
      ...
      <div class="modal-body">
        <form action="../assets/report/rbarangrusak.php" target="_blank" method="post">
        <div class="input-group input-group-mb" style="margin-bottom: 10px">
          ...
          <select name="bulan" class="form-control bg-dark" required style="color: white; font-weight: bold;">
            <?php
              $ahay = mysqli_query($kon, "SELECT MONTH(tgl) as bulan FROM barangrusak GROUP BY bulan ORDER BY bulan ASC");
          ...
          <?php
              $ahay = mysqli_query($kon, "SELECT YEAR(tgl) as tahun FROM barangrusak GROUP BY tahun ORDER BY tahun ASC");
              while($baris = mysqli_fetch_array($ahay)) {
              $tahun = $baris['tahun']; 
          ...
          <div class="col-12">
            <div class="card">
              <div class="card-header">
                ...
                <button style="float: right;margin-left: 5px" class="btn btn-primary" type="button" data-toggle="tooltip" data-placement="bottom" title="Tambah"><a href="barangrusak_input.php" class="text-white"><i class="fas fa-folder-plus"></i></a>
                </button>
              ...
              <div class="card-body">
                <table id="dataTables" class="table table-bordered table-hover table-sm">
                  ...
                  <tbody>
                    <?php 
                      $no = 1;
                      $query = mysqli_query($kon, "SELECT * FROM barangrusak INNER JOIN barang ON barangrusak.idbarang = barang.idbarang ORDER BY tgl DESC");
                      while($data = mysqli_fetch_array($query)){
                        ?>
                          ...
                          <td><?= $data['ket'] ?></td>
                          <td>
                            <button class="btn bg-dark" type="button"><a href="barangrusak_edit.php?idbarangrusak=<?= $data['idbarangrusak'] ?>" class="text-white"><i class="far fa-edit"></i></a></button>
                            <button class="btn bg-dark" onclick="yakin = confirm('Apakah Kamu yakin ingin Menghapus?'); if(yakin){ window.location = 'hapus.php?idbarangrusak=<?= $data['idbarangrusak'] ?>';
