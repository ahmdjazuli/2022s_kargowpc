              ...
              <div class="card-body">
                <table id="dataTables" class="table table-bordered table-hover table-sm">
                  ...
                  <tbody>
                    <?php 
                      $no = 1;
                      $query = mysqli_query($kon, "SELECT * FROM qc JOIN barangmasuk ON qc.idbarangmasuk = barangmasuk.idbarangmasuk JOIN barang ON barangmasuk.idbarang = barang.idbarang JOIN user ON qc.id = user.id ORDER BY tglqc DESC");
                      while($data = mysqli_fetch_array($query)){
                        ?>
                          ...
                          <td>
                            ...
                            <button class="btn bg-dark" onclick="yakin = confirm('Apakah Kamu yakin ingin Menghapus?'); if(yakin){ window.location = 'hapus.php?idqc=<?= $data['idqc'] ?>';
